...
DEFAULT_UPLINK_CONVERTER = 'BytesPsUplinkConverter'
...
class PsConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        self.setName(config.get("name", 'PS Connector ' + ''.join(choice(ascii_lowercase) for _ in range(5))))
        self.daemon = True
        ...
        self.__socket = socket.socket(socket.AF_INET, config.get("type"))
